...
<template>
  <div class="page">
    <header class="top"><h1>Tài khoản</h1></header>
    ...
    <main class="container">
      ...
      <div v-if="loading" class="skeleton">
        ...
        <div class="sk-list"><div class="sk-item"></div><div class="sk-item"></div><div class="sk-item"></div></div>
      </div>

      <!-- Error -->
      <div v-else-if="errorMsg" class="error">
        <i class="bi bi-exclamation-triangle"></i><p>{{ errorMsg }}</p>
      </div>
      ...
      <div v-else class="stack">
        ...
        <section class="card user">
          ...
          <div class="uinfo">
            ...
            <div class="chip">UID: {{ profile?.id }}</div>
        ...
        <section class="grid-2">
          <div class="stat htw">
            ...
            <div class="value">
              {{ (profile?.htw_balance ?? 0).toLocaleString() }} <span>HTW</span>
            </div>
          </div>
          <div class="stat vnd">
            <div class="label">VND</div>
            <div class="value">
              {{ (profile?.vnd_balance ?? 0).toLocaleString() }} <span>VND</span>
            </div>
          </div>
        </section>
...
.page{
  --bg:#0b0f1a;--card:#101826;--card-2:#0d1422;--text:#e5e7eb;--muted:#9aa3b2;
  --ring:0 0 0 1px rgba(148,163,184,.14);
  background:var(--bg);color:var(--text);
  width:100dvw; min-height:100dvh; display:flex; flex-direction:column;
}
...
.container{flex:1;width:100%;padding:12px 16px calc(96px + env(safe-area-inset-bottom))}
.stack{display:grid;gap:14px}
.card{background:var(--card);border-radius:16px;padding:14px;box-shadow:var(--ring)}
.user{display:grid;grid-template-columns:72px 1fr;gap:12px;align-items:center}
...
.avatar{width:72px;height:72px;border-radius:50%;display:grid;place-items:center;
  background:linear-gradient(145deg,#1f2937,#0b1220);border:1px solid rgba(148,163,184,.18);
  font:800 22px/1 ui-sans-serif,system-ui}
.online{position:absolute;right:4px;bottom:4px;width:14px;height:14px;border-radius:50%;
  background:#10b981;border:2px solid var(--card);box-shadow:0 0 0 2px rgba(16,185,129,.25)}
.uinfo .name{font-weight:700;font-size:16px;margin-bottom:6px}
.chip{display:inline-block;font:12px ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;
  color:var(--muted);background:var(--card-2);padding:4px 8px;border-radius:8px;
  border:1px solid rgba(148,163,184,.12)}
.handle{color:#60a5fa;font-size:13px;margin-top:6px}
.grid-2{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.stat{border-radius:16px;padding:14px;position:relative;overflow:hidden;
  border:1px solid rgba(148,163,184,.12)}
.stat::after{content:"";position:absolute;inset:-30% -20% auto auto;width:120px;height:120px;
...
.item{display:grid;grid-template-columns:44px 1fr 18px;align-items:center;gap:12px;
  padding:12px;border-radius:14px;background:var(--card);
  border:1px solid rgba(148,163,184,.12);text-align:left}
.item:hover{background:#0f1a2a}
.ic{width:44px;height:44px;border-radius:12px;display:grid;place-items:center;color:#fff;font-size:18px}
