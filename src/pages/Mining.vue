...
const chest = ref({ reward: 14, cooldown: 1200, remaining: 0, today: 0, limit: 30 }) // NEW: today/limit
  ...
  try {
    ...
    if (!r.ok || data?.ok !== true) {
      ...
      chestMsg.value = data?.ok === false
        ? (chest.value.today >= chest.value.limit ? 'Hôm nay đã đủ 30 lần.' : 'Chưa hết thời gian chờ rương.')
        : 'Mở rương thất bại.'
      return
    }
