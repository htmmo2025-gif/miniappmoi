...
const REWARD   = Number(process.env.WATCH_MONTAG_REWARD_HTW ?? 2)
const COOLDOWN = Number(process.env.WATCH_MONTAG_COOLDOWN_SEC ?? 600)
const DAILY    = Number(process.env.WATCH_MONTAG_DAILY_LIMIT ?? 10)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const { data, error } = await supa.rpc('get_montag_status', {
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await supa.rpc('claim_montag_reward', {
    ...
    console.error('watchadsmontag error:', e)
